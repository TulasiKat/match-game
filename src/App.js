...
class App extends Component {
  ...
  thumbnailClickedFunction = (randomNumber, id) => {
    ...
    this.setState({mainImage: imagesList[randomNumber]})
  }

  playAgainClicked = () => {
    this.setState({gameRunning: true, score: 0, time: 60})
    this.componentDidMount()
  }

  render() {
    const {score, time, mainImage, selectedTab, gameRunning} = this.state
    const results = imagesList.filter(each => each.category === selectedTab)
      ...
      <div className="main-container">
        ...
        {gameRunning ? (
          <div className="main-part-container">
            <img src={mainImage.imageUrl} alt="match" className="main-image" />
            ...
            <ul className="images-container">
              {results.map(each => (
                <Thumbnail
                  details={each}
                  key={each.id}
                  thumbnailClickedFunction={this.thumbnailClickedFunction}
                />
              ))}
            </ul>
          </div>
        ) : (
          <div className="game-over-container main-part-container">
            <img
